...
message message::new_signal(const bus &conn, const std::string &path, const std::string &interface, const std::string &member) {
    message msg;
    sd_bus_message_new_signal(conn.d_ptr_->conn_, &msg.d_ptr_->message_, path.c_str(), interface.c_str(), member.c_str());
